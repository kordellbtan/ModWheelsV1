...
const uint8_t CMD_LIST_SIZE = 4;   // we are adding 3 commands (MOVE, BLINK, SERVO1, SERVO2)
...
void servoHandler (uint8_t cmd, uint8_t param[])
{
    ...
    servo11.write(param[1]);       // turn servo to specified angle
    delay(1500);
    servo11.detach();              // detach so servo is not continuously affected by PWM
    
}  // servoHandler

void servoHandler2 (uint8_t cmd, uint8_t param[], uint8_t n)
{
    Serial.write(cmd);             // servo command = 0x42
    Serial.write(n);               // number of param = 1
    for (int i=0;i<n;i++)          // param = 90 degrees
    {
        Serial.write(param[i]);
    }
    servo13.attach(13);            // attach servo to pin 11
    servo13.write(param[0]);       // turn servo to specified angle
    servo13.detach();              // detach so servo is not continuously affected by PWM
...
ArxRobot::cmdFunc_t onCommand[CMD_LIST_SIZE] = {{MOVE,moveHandler}, {BLINK,blinkHandler}, {SERVO,servoHandler}, {SERVO2,servoHandler2}};
...
{
  ServoTest();
  motorPWM.setAccuracy(.3);          // change sensor accuracy from +/- 2 DN to +/- 1 DN
  motorPWM.setSamplePeriod(500);    // sample period from 1 second to 0.5 seconds
